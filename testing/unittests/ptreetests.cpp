...
<Dataset _uid='u1' type="1" global_id="1000">
    ...
    <EdgeCases _uid='u82'>
        <StringSort _uid='u83' val="20"/>
        <StringSort _uid='u84' val="3"/>
        <DeepMatch _uid='u85'>
            <Node _uid='u86' id="99"/>
        </DeepMatch>
    </EdgeCases>
...
class PTreeXPathTests : public CppUnit::TestFixture
{
    ...
    void checkXPathException(IPropertyTree * root, const char * testName, size_t testNumber, const char * queryXPath)
    {
        try
        {
            root->queryProp(queryXPath);
            VStringBuffer msg("Test '%s' [#%zu]: Expected exception for query '%s', but none was thrown.", testName, testNumber, queryXPath);
            CPPUNIT_FAIL(msg.str());
        }
        catch (IException * e)
        {
            e->Release();
            // Exception successfully caught as expected
        }
        ...
        {
            VStringBuffer msg("Test '%s' [#%zu]: Expected IException for query '%s', but a different type was thrown.", testName, testNumber, queryXPath);
            CPPUNIT_FAIL(msg.str());
        }
    }
    ...
    void testEdgeCases(IPropertyTree * root)
    {
        XPathTestCase edgeCaseTests[] = {
            // Nested sub-path in qualifier
            { "EdgeCases", "DeepMatch[Node/@id=\"99\"]/Node/@id", "99" },
            // "." resolution
            //MORE: This should probably work, but currently fails.
//            { nullptr, "./ArrayItems/./Item[1]", "29" },
            // Nested identical brackets logic
            { nullptr, "EdgeCases[DeepMatch][1]/StringSort[2]/@val", "3" },
            // Whitespace inside bracket filters is allowed and trimmed
            { nullptr, "EdgeCases/StringSort[   @val  =  \"20\"   ]/@val", "20" },
            // Quoted vs unquoted numeric matches (unquoted treats mathematically: 3 < 20. Quoted treats lexically: "3" > "20")
            // wait, if I use "val<10", 3 should match.
            { "EdgeCases", "StringSort[@val<10]/@val", "3" },
            // Default wildcard behavior on "=" (no ~ needed for literal string matching wildcard format)
            { "Types", "StringTest[@status=\"Act*\"]/@mixed", "aCtIvE" }
        };
        runXPathTests(root, "XPath Edge Cases", edgeCaseTests, std::size(edgeCaseTests));

        CCycleTimer timer;
        const char* exceptionTests[] = {
            // ".." is officially completely unsupported in jptree and explicitly verified by switch cases
            "ArrayItems/../Default",
            // Functional tests inside the node syntax like text() natively violate the parser's restricted character definitions
            "Record()",
            "Record/Flags()",

            //The following are not caught because text and count do not exist.
//            "Record/text()",
//            "Types/count()"
        };

        for (size_t i = 0; i < std::size(exceptionTests); i++)
        {
            checkXPathException(root, "XPath Exceptions", i, exceptionTests[i]);
        }

        DBGLOG("Test XPath Edge Cases Exception Checks took %u ms", timer.elapsedMs());
    }
