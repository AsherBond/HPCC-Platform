...
IF ("${COMMONSETUP_DONE}" STREQUAL "")
  ...
  if (CMAKE_COMPILER_IS_CLANGXX)
    ...
    if ("${clang_full_version_string}" MATCHES "emcc \\(Emscripten gcc/clang-like replacement \\+ linker emulating GNU ld\\) ([0-9]+\\.[0-9]+\\.[0-9]+).*")
      string (REGEX REPLACE "emcc \\(Emscripten gcc/clang-like replacement \\+ linker emulating GNU ld\\) ([0-9]+\\.[0-9]+\\.[0-9]+).*" "\\1" EMSCRIPTEN_CLANG_VERSION "${clang_full_version_string}")
    elseif ("${clang_full_version_string}" MATCHES "Apple LLVM version ([0-9]+\\.[0-9]+\\.[0-9]+).*")
      string (REGEX REPLACE "Apple LLVM version ([0-9]+\\.[0-9]+\\.[0-9]+).*" "\\1" APPLE_CLANG_VERSION "${clang_full_version_string}")
    elseif ("${clang_full_version_string}" MATCHES ".*based on LLVM ([0-9]+\\.[0-9]+).*")
      string (REGEX REPLACE ".*based on LLVM ([0-9]+\\.[0-9]+).*" "\\1" CLANG_VERSION "${clang_full_version_string}")
    elseif ("${clang_full_version_string}" MATCHES ".*clang version ([0-9]+\\.[0-9]+).*")
  ...
  if (WIN32)
    ...
    if (CMAKE_COMPILER_IS_CLANGXX)
      ...
      SET (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Werror=reorder-ctor")
      endif()
